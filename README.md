...
มี 2 วิธี ใช้คู่กันได้:

- **B (เร็ว):** กด checkbox ในหน้า `PROGRESS.html` ได้เลย — จำใน browser (localStorage) ปิดเปิดใหม่ยังอยู่ เหมาะตอนเรียนจริง
- **A (เก็บถาวร):** sync ลง markdown สำหรับเก็บเป็น portfolio ใน git. เปลี่ยน `[ ]` → `[x]` ใน `PROGRESS.md` แล้ว `node tools/build-html.js` + `git commit`
