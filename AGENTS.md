...
- **NEVER use `curl` to test whether prod is blocked — it always fails, ban or no ban.** Akamai fingerprints the TLS/HTTP2 client, and curl's fingerprint is rejected outright at the connection layer: `curl: (92) HTTP/2 stream 1 was not closed cleanly: INTERNAL_ERROR`, exit status `000`. Forcing `--http1.1` does not help, and even `robots.txt` fails, so the failure looks exactly like a network-level IP ban. On 2026-08-05 this cost hours: curl reported `000` for every prod URL and the session concluded the IP was banned, while the same URLs returned **HTTP 200 in the project's own puppeteer**, headless and headful alike (5/5 pages, including one that had genuinely been serving Access Denied earlier that day). **Probe with the same tool the pipeline uses** — launch `puppeteer-core` with `CHROME_EXECUTABLE_PATH` and `CAPTURE_USER_AGENT` and call `page.goto()`. A curl result says nothing about prod's state. The proper probe is now `npm run probe` (src/waf-probe.js — exit 0 ok / 1 blocked, writes data/waf-status.json); `--watch` for continuous monitoring; compare.js pre-flights automatically unless `--no-wait`.
- **WAF-era recapture pattern that works:** probe-gated chunks of 1-3 pages (`npm run probe` first), jittered 15-75 min cooldowns, blocked ids re-queued to the TAIL of the queue — not one long run. And `SHARE_BROWSER_CONTEXT` must stay `false`: flipping it to true (to reuse Chrome's cache, −64% requests) got a run blocked from page 2 — shared cookies let the WAF correlate every page into one bot session (2026-08-06; measured isolated 3/6 vs shared 0/6).
